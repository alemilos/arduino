...
void SimulatedDataSource::_advanceSimulation(uint32_t dtMs) {
    ...
    if (_simPhase == 0) {
        ...
        _data.rpmEngine = 800 + _data.speedKmh * 60;
        if (_phaseCounter >= 5000) { _simPhase = 1; _phaseCounter = 0; }
    }
    // Fase 1: crociera (10 secondi)
    else if (_simPhase == 1) {
        _data.speedKmh  = 30;
        _data.rpmEngine = 1800 + (uint16_t)(_phaseCounter % 200); // leggera oscillazione
    ...
    _data.batteryVoltage = 12.6f - (_data.consumedMah / 10000.0f);
...
float SimulatedDataSource::_computeEfficiency() const {
    ...
    float consumedKwh = (_data.consumedMah * _data.batteryVoltage) / 1000000.0f;
